...
//    using func_t = void (*)(RefObject *ptr, void *arg);
    using func_t = std::function<void(RefObject *ptr, void *arg)>; ///< 删除函数类型，使用std::function支持更灵活的函数签名
    func_t deleter_; ///< 删除函数指针
    void *arg_; ///< 删除函数参数
    bool empty_;

    /// @brief 默认构造函数：空删除器
    explicit Deleter() noexcept: deleter_(nullptr), arg_(nullptr), empty_(true)  ///< 初始化为空删除器
    {}

    /// @brief 带参构造函数：绑定释放函数和参数
    /// @param func 自定义释放函数
    /// @param arg 传递给释放函数的参数
    Deleter(func_t func, void *arg) : deleter_(func), arg_(arg), empty_(false)  ///< 初始化为非空删除器
    {}
...
public:
    ...
    /// 默认使用共享模式，引用计数指针指向自身
    explicit RefObject()
            : refCount_(&refCountValue_), refCountValue_{1}
    {
    }
    ...
    {
        if (IsShared()) {
            refCountValue_.fetch_add(1, std::memory_order_relaxed);  // relaxed模式下增加引用计数,可以保证对所有acquire可见
        } else {
    ...
    {
        if (IsShared()) {
            if (refCountValue_.fetch_sub(1, std::memory_order_seq_cst) == 1) {
                if (deleter_.empty_) {
            ...
            if (refCount_->fetch_sub(1, std::memory_order_seq_cst) == 1) {
                if (deleter_.empty_) {
                    delete this;  ///< 如果没有自定义删除器，直接删除对象
                }
                else {
    ...
    {
        if (IsShared()) {
            return refCountValue_.load(std::memory_order_relaxed);  ///< relaxed模式下获取引用计数
        } else {
...
public:
    atomic_t<std::size_t> *refCount_; ///< 引用计数指针(指向自身或者其他对象的引用计数) 共享模式则指向自身
    atomic_t<std::size_t> refCountValue_; ///< 非共享模式下的本地引用计数（默认初始化1）
...
public:
    ...
    explicit RefObjectImpl() noexcept: reference_(1), weak_(1)
    {}

    /// @brief 增加弱引用计数
    void AddWeakRef()
    {
//        weak_.fetch_add(1, std::memory_order_relaxed);  // relaxed模式下增加弱引用计数
        ++weak_;
    }
...
//            delete this;
//        }
//        return true;  ///< 弱引用计数减为0时删除对象
        if (weak_.fetch_sub(1, std::memory_order_seq_cst) == 1) { // fetch_sub返回减去1之前的值
    ...
    bool Lock()
    {
        // 使用 acquire 语义加载计数，确保之前的操作对当前线程可见
        std::size_t count = reference_.load(std::memory_order_acquire);

        do {
            // 对象已销毁，无法升级
            if (count == 0) return false;

            // 使用 acq_rel 内存序确保原子性和内存可见性
        } while (!reference_.compare_exchange_weak(
                count, count + 1,
                std::memory_order_acq_rel,    // 成功时使用获取-释放语义
                std::memory_order_relaxed     // 失败时仅保证原子性
        ));

        return true;
    }

    /// @brief 禁止拷贝构造和赋值（仅通过智能指针管理）
    RefObjectImpl(RefObjectImpl const &) = delete;

    RefObjectImpl &operator=(RefObjectImpl const &) = delete;

private:
    atomic_t<std::size_t> reference_;  ///< 强引用计数（关联IncursivePtr）
    atomic_t<std::size_t> weak_;       ///< 弱引用计数（关联WeakPtr）
...
public:
    ...
    /// @brief 析构函数：减少强引用计数
    virtual ~SharedRefWrapper()
    {
        if (impl_ && impl_->DecWeakRef()) {
            delete impl_;  ///< 删除实现对象
        }
    }
    ...
    bool DecRef() override
    {
        ...
        if (RefObject::DecRef()) {  ///< 先减少强引用计数，此处强引用绑定到了 impl_->reference_
            std::atomic_thread_fence(std::memory_order_acq_rel);  ///< 内存屏障，保证顺序性
            impl->DecWeakRef();  ///< 释放弱引用计数块
            return true;
        }
        return false;
    }
...
public:
    ...
    /// @brief 拷贝赋值运算符：共享对象，增加引用计数，注意拷贝前先释放当前对象，因为可能已经持有一个对象
    /// @param other 源智能指针
    ...
    /// @brief 移动赋值运算符：转移所有权，减少引用计数，源对象不再持有！！！！
    /// @param other 源智能指针
    ...
    /// @brief 判断是否唯一持有对象
    /// @return true：唯一持有；false：存在其他持有者
    bool unique()
    {
        return UserCount() == 1;
    }
...
//    SharedRefWrapper* wrapper_;  ///< 引用计数包装器
...
public:
    ...
    /// @brief 尝试将弱引用观察-->升级为强引用（获取对象当前状态）
    /// @return 有效的IncursivePtr（对象存活时）或空指针（对象已销毁）
    IncursivePtr<T> Lock() const {
        if (!impl_) return IncursivePtr<T>();  ///< 无Impl说明对象非共享或已销毁
        if (!impl_->Lock()) return IncursivePtr<T>();  ///< 尝试增加强计数，失败则对象已销毁
        IncursivePtr<T> iptr(ptr_);  ///< 创建强指针
        ptr_->AddRef();  ///< 平衡强指针构造时的计数增加
        return iptr;
    }
    ...
    explicit operator bool() const {
        return !!impl_;  ///< 有Impl即表示关联到共享对象
    }


private:
    RefObjectImpl *impl_;  ///< 引用计数实现对象指针
    T* ptr_;  ///< 弱指针指向的对象指针，这里支持IncursivePtr<T>，还有SharedRefWrapper等
//    SharedRefWrapper *wrapper_;  ///< 引用计数包装器
};
...
template <typename T>
typename std::enable_if<std::is_base_of<RefObject, T>::value, std::shared_ptr<T>>::type // 如果为RefObject的子类，则启用此函数，否则禁用
SharedWrapper(T *ptr)
{
    if (!ptr) {
